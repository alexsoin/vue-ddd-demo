...
export class EmployeeService {
   ...
   */
	async createEmployee(input: AggregatedEmployee): Promise<any> {
		...
		return await this.employeeRepository.save({ ...input, id });
	}

	/**
   * Обновить сотрудника
   */
	async updateEmployee(id: string, patch: Partial<AggregatedEmployee>): Promise<any> {
		...
		return await this.employeeRepository.update(merged);
	}
   ...
   */
	async listEmployees(filter?: EmployeeFilter): Promise<any[]> {
   ...
   */
	async reassignDepartment(employeeId: string, departmentId: string | null): Promise<any> {
		...
		return await this.employeeRepository.update({ ...emp, departmentId });
	}
}
